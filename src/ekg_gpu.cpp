...
void ekg_gpu_data_handler::scissor(int32_t x, int32_t y, int32_t w, int32_t h) {
    ...
    scissor.x = x;
    scissor.y = (static_cast<int32_t>(ekg::core->get_screen_height())) - (y + h);
    scissor.w = w;
    scissor.h = h;
}
...
void ekggpu::rectangle(float x, float y, float w, float h, ekgmath::vec4f &color_vec, int32_t line_thickness) {
    // Alloc arrays in CPU.
    ekggpu::push_arr_rect(ekg::core->get_gpu_handler().get_cached_vertices(), 0.0f, 0.0f, 1.0f, 1.0f);
...
void ekggpu::invoke() {
    ekg::core->get_gpu_handler().start();
}

void ekggpu::revoke() {
    ekg::core->get_gpu_handler().end();
}

void ekggpu::bind_scissor(int32_t index) {
    ekg::core->get_gpu_handler().bind_scissor(index);
}

void ekggpu::scissor(int32_t x, int32_t y, int32_t w, int32_t h) {
    ekg::core->get_gpu_handler().scissor(x, y, w, h);
}
...
int32_t ekggpu::start_scissor() {
    return ekg::core->get_gpu_handler().get_scissor_id();
}

void ekggpu::end_scissor() {
    ekg::core->get_gpu_handler().end_scissor();
}

void ekggpu::next_scissor() {
    ekg::core->get_gpu_handler().next_scissor();
}
