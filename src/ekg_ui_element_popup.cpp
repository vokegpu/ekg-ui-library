...
void ekg_popup::destroy() {
    ...
    if (this->has_mother() && ekg::core->find_element((ekg_element*&) popup, this->mother_id) && !popup->access_flag().dead) {
        popup->destroy();
    }

    ekg::core->kill_element(this);

    for (uint32_t &ids : this->children_stack.ids) {
        if (ekg::core->find_element((ekg_element*&) popup, ids)) {
            popup->destroy();
        }
    }
}
...
void ekg_popup::on_event(SDL_Event &sdl_event) {
    ...
    if (ekgapi::motion(sdl_event, mx, my)) {
        highlight = this->flag.over || !this->flag.focused || (this->has_mother() && this->mother_id == ekg::core->get_popup_top_level());
    } else if (ekgapi::any_input_down(sdl_event, mx, my) && this->flag.focused) {
        bool flag = ekg::core->get_hovered_element_type() != ekg::ui::POPUP;
    ...
    if (highlight) {
        ...
        if (this->flag.focused) {
            ...
            for (ekgutil::component &component : this->component_list) {
                if (ekgmath::collide_aabb_with_point(this->rect.x, this->rect.y + this->full_height, component.w, component.h) && component.enabled) {
                    ...
                    ekg::core->get_popup_top_level() = component.data != 0 ? this->id : this->mother_id;
                }
        ...
        if (this->has_mother() && ((!this->flag.focused) || (
                ekg::core->get_popup_top_level() == this->mother_id && !this->flag.over))) {
...
void ekg_popup::get_popup_path(std::string &path) {
    if (this->has_mother()) {
        ...
        ekg_popup* element;
        ekg::core->find_element((ekg_element*&) element, this->mother_id);
