...
bool ekgapi::input_down_right(SDL_Event &sdl_event, float &x, float &y) {
    switch (sdl_event.type) {
        ...
        case SDL_FINGERDOWN: {
            x = sdl_event.tfinger.x * ekg::core->get_screen_width();
            y = sdl_event.tfinger.y * ekg::core->get_screen_height();
...
void ekgapi::callback_popup(uint32_t id, const std::string &path) {
    ...
    ekg::core->dispatch_event(sdl_custom_event);
}
...
void ekgapi::callback_combobox(uint32_t id, const std::string &text) {
    ...
    ekg_custom_event->id = id;
    ekg_custom_event->type = ekg::ui::COMBOBOX;
    ekg_custom_event->text = text;
...
void ekgapi::callback_frame(uint32_t id, const std::string &text) {
    ...
    ekg_custom_event->id = id;
    ekg_custom_event->type = ekg::ui::FRAME;
    ekg_custom_event->text = text;
